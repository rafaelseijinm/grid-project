...
def grid_gen(size):
    ...
    # Adding roads with different sizes (1 or 2)
    for i in range(size):
        if i % 10 == 0:
...
def is_intersection_or_change(grid, y, x):
    ...
    # Seeing if its an intersection, finish or width change
    if len(neighbors) != 2:
        return True
...
def graph_create(grid):
    ...
    # nodes: buildings, intersections and changes
    nodes = {}
    for y in range(size):
        for x in range(size):
            cell = grid[y][x]
    ...
    for y in range(size):
        for x in range(size):
            if grid[y][x]["type"] == "road":
                ...
                # search for next node (vertical or horizontal)
